...
`;
Video.Content = styled(Box)``;
Video.Title = styled(Heading)`
  font-size: 32px;
  font-weight: 500;
...
`;
Video.Subtitle = styled(Heading)`
  font-size: 16px;
  font-weight: 500;
  letter-spacing: normal;
  line-height: 1.75;
`;
Video.Text = styled(Paragraph)`
  font-size: 16px;
  font-weight: 400;
  letter-spacing: normal;
  line-height: 1.77777;
`;
Video.ButtonGroup = styled(Box)`
  margin-right: -17.0656px;
  display: flex;
  flex-wrap: wrap;
`;

Video.Image = styled(Box)`
  .video-btn {
    display: flex;
    align-items: center;
